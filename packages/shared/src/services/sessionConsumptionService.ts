...
export class SessionConsumptionService {
  ...
  /**
   * Cleanup expired sessions (moved to collection, no longer needed here)
   * @deprecated This is now handled by the cron job
   */
  static async cleanupExpiredSessions(userId: string): Promise<boolean> {
    // This functionality is now handled by the cron job in api/cron/notifications.js
    // No need to implement here as sessions are managed in the collection
    console.warn('cleanupExpiredSessions is deprecated - handled by cron job');
    return true;
  }
}
