...
if (!AI_ENDPOINT) {
  logger.error("Aucun endpoint ARCHA configuré. ARCHA ne fonctionnera pas.", undefined, 'DirecteurChat');
}
...
- Référence les données sources`;
    ...
    if (!conversationId) {
      try {
        ...
        logger.error('Error creating conversation', error, 'DirecteurChat');
    ...
    if (currentConversation) {
      try {
        ...
        logger.error('Error adding user message to local state', error, 'DirecteurChat');
      }
    }
    ...
    try {
      ...
      if (response.status === 401) {
        try {
          ...
          logger.error('Failed to refresh token', refreshErr, 'DirecteurChat');
        }
      }
      ...
      if (data.userMessage && currentConversation) {
        try {
          ...
          logger.error('Error replacing optimistic user message', error, 'DirecteurChat');
        }
      }
      ...
      if (user && data.meta?.userTokensCharged) {
        ...
        try {
          setTimeout(async () => {
            try {
              ...
              logger.error('Failed to refresh user data after token deduction', refreshError, 'DirecteurChat');
            }
          }, 1000); // 1 second delay to allow UI to settle
        } catch (refreshError) {
          logger.error('Failed to schedule user data refresh', refreshError, 'DirecteurChat');
        }
        ...
        try {
          ...
          logger.error('Failed to log chat activity', analyticsError, 'DirecteurChat');
        }
      }
      ...
      if (currentConversation) {
        try {
          ...
          logger.error('Error removing loading message', error, 'DirecteurChat');
        }
      }

    } catch (error) {
      logger.error('Erreur lors de l\'envoi du message', error, 'DirecteurChat');
      ...
      if (currentConversation) {
        try {
          ...
          logger.error('Error adding error message to local state', error, 'DirecteurChat');
        }
      }
